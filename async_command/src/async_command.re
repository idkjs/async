...
let maybe_print_error_and_shutdown =
  fun
  | Ok () => shutdown(0)
...
let in_async = (~extract_exn=?, param, on_result) =>
  Param.map(
    param,
    ~f=(staged_main, ()) => {
      let main = Or_error.try_with(() => unstage(staged_main()));
      switch (main) {
      | Error(e) =>
      ...
      | Ok(main) =>
        let before_shutdown = () =>
          Deferred.List.iter(
            ~how=`Parallel, Writer.[force(stdout), force(stderr)], ~f=writer =>
            Deferred.any_unit([
        ...
        Shutdown.set_default_force(
          {
            let prev = Shutdown.default_force();
            () =>
              Deferred.all_unit([
                ...
                prev(),
                before_shutdown() >>= (() => after(sec(1.))),
              ]);
          },
        );
        upon(
          Deferred.Or_error.try_with(
            ~run=`Schedule, ~rest=`Log, ~extract_exn?, () =>
...
module Staged = {
  let async = (~extract_exn=?, ~summary, ~readme=?, param) => {
    let on_result = maybe_print_error_and_shutdown;
  ...
  let async_or_error = (~extract_exn=?, ~summary, ~readme=?, param) => {
    let on_result = res =>
      maybe_print_error_and_shutdown(Or_error.join(res));
    basic(~summary, ~readme?, in_async(~extract_exn?, param, on_result));
  };
...
let stage_param =
  Param.map(~f=(main, ()) => stage((`Scheduler_started) => main()));
