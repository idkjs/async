...
let test = () =>
  ...
  >>= (
    ((`Reader(reader_fd), `Writer(writer_fd))) => {
      let string_writer = Writer.create(writer_fd);
      ...
      List.iter(sexps, ~f=sexp =>
        Writer.write(string_writer, Sexp.to_string(sexp))
      );
      ...
      let rec loop_sexps = sexps =>
        ...
        | [sexp, ...sexps] =>
          let packed = Sexp.to_string(sexp);
          let rec loop_bytes = i =>
            if (i == String.length(packed)) {
              ...
              Writer.write(
                string_writer,
                String.sub(packed, ~pos=i, ~len=1),
              );
              after(sec(0.001)) >>> (() => loop_bytes(i + 1));
            };
      ...
      loop_sexps(sexps);
      let all = Pipe.read_all(sexp_reader);
      unpack_result
      >>= (
        result => {
          switch (result) {
