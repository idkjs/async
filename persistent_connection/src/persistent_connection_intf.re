...
module type S = {
  ...
  let create:
    (
      ~server_name: string,
      ~log: /** If [~log] is supplied then all events that would be passed to [on_event] will be
        written there as well, with a "persistent-connection-to" tag value of
        [server_name], which should be the name of the server we are connecting to. */ Log.t
              =?,
      ~on_event: Event.t => Deferred.t(unit)=?,
      ~retry_delay: unit => Time.Span.t=?,
      ~random_state: /** If a [~random_state] is supplied, randomization is applied to the result of
...
module type S_rpc = {
  ...
  let create':
    (
      ...
      ~on_event: Event.t => Deferred.t(unit)=?,
      ~retry_delay: unit => Time.Span.t=?,
      ~random_state: [ | `Non_random | `State(Random.State.t)]=?,
      ~time_source: Time_source.t=?,
...
module type Persistent_connection = {
  ...
  module Make:
    (Conn: T) => S with type conn = Conn.t and type address = Conn.Address.t;
  module Rpc: S_rpc with type conn = Rpc.Connection.t;
  module Versioned_rpc:
    S_rpc with type conn = Versioned_rpc.Connection_with_menu.t;
};
