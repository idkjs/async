...
let dispatch_exn = (rpc, {Host_and_port.host, port}, arg) =>
  Rpc.Connection.with_client(
    Tcp.Where_to_connect.of_host_and_port({host, port}), conn =>
    Rpc.Rpc.dispatch_exn(rpc, conn, arg)
  )
  >>| Result.ok_exn;
...
let set_id_counter = (addr, new_id) =>
  dispatch_exn(Rpc_intf.set_id_counter, addr, new_id);

let set_id_counter_v0 = (addr, new_id_pair) =>
  dispatch_exn(Rpc_intf.set_id_counter_v0, addr, new_id_pair);

let get_unique_id = addr => {
  let%map id = dispatch_exn(Rpc_intf.get_unique_id, addr, ());
  printf("UNIQUE ID: %d\n", id);
};

let counter_values = addr =>
  pipe_dispatch_exn(Rpc_intf.counter_values, addr, (), reader =>
    Pipe.iter_without_pushback(reader, ~f=i => printf("COUNTER: %d\n%!", i))
  );
...
let set_id_counter_cmd =
  Command.async(
    ~summary="forcibly set the unique id counter.  DANGEROUS",
    {
      ...
      and i = anon("counter" %: int);
      () => set_id_counter(host_and_port, i);
    },
  );
...
let set_id_counter_cmd_v0 =
  Command.async(
    ~summary="forcibly set the unique id counter.  DANGEROUS",
    {
      ...
      and id2 = anon("counter2" %: int);
      () => set_id_counter_v0(host_and_port, (id1, id2));
    },
  );
