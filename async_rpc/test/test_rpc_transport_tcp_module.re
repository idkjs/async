...
let implementations =
  Rpc.Implementations.create_exn(
    ...
    ~implementations=[
      Rpc.Rpc.implement'(string_string_rpc, (state, query) =>
        state ++ " " ++ query
      ),
    ],
  );
...
let%expect_test "basic functionality" = {
  let server_listening_on_set_once = Set_once.create();
  let%bind server =
    Rpc.Transport.Tcp.serve(
      ...
      (~client_addr, ~server_addr, transport) => {
        ...
        assert(
          ![%compare.equal: Socket.Address.Inet.t](server_addr, client_addr),
        );
        /* we can create a connection using this transport */
        let%bind connection =
          Async_rpc_kernel.Rpc.Connection.create(
            ~implementations,
            ~connection_state=_ => "server-state",
            transport,
          );

        let connection = Result.ok_exn(connection);
        Rpc.Connection.close_finished(connection);
      },
    );

  let server_listening_on = Tcp.Server.listening_on_address(server);
  Set_once.set_exn(
    server_listening_on_set_once,
    [%here],
    server_listening_on,
  );
  /* we can connect to the server */
  let%bind client_transport =
    Rpc.Transport.Tcp.connect(
      Tcp.Where_to_connect.of_inet_address(server_listening_on),
    );

  let (client_transport, server_addr) = Result.ok_exn(client_transport);
  ...
  let client_conn = Result.ok_exn(client_conn);
  let%bind result =
    Rpc.Rpc.dispatch_exn(string_string_rpc, client_conn, "query");
  print_endline(result);
  %expect
  {| server-state query |};
  let%bind () = Rpc.Connection.close(client_conn);
  let%bind () = Tcp.Server.close(server);
  return();
};

let with_timeout = (~context, deferred) => {
  let generous_timeout_for_hydra = sec(30.);
  switch%map (Clock.with_timeout(generous_timeout_for_hydra, deferred)) {
...
let%expect_test "transport is closed when handle_transport returns #1" = {
  let server_transport = Ivar.create();
  let%bind server =
    Rpc.Transport.Tcp.serve(
      ~where_to_listen,
      (~client_addr as _, ~server_addr as _, transport) => {
  ...
  let%bind (_sock, reader, writer) =
    Tcp.connect(
      Tcp.Where_to_connect.of_inet_address(
        Tcp.Server.listening_on_address(server),
      ),
    );

  let%bind server_transport = Ivar.read(server_transport);
  /* the transport is closed immediately, so the client reader returns EOF. */
  let%bind result = Reader.peek(reader, ~len=1);
  print_s([%sexp (result: Reader.Read_result.t(string))]);
  %expect
  {| Eof |};
...
let%expect_test "transport is closed when handle_transport returns #2" = {
  let server_connection = Ivar.create();
  let server_has_finished_with_connection = Ivar.create();
  let%bind server =
    Rpc.Transport.Tcp.serve(
      ~where_to_listen,
      (~client_addr as _, ~server_addr as _, transport) => {
  ...
  let client_connection = Result.ok_exn(client_connection);
  let%bind server_connection = Ivar.read(server_connection);
  assert(!Rpc.Connection.is_closed(client_connection));
  assert(!Rpc.Connection.is_closed(server_connection));
